"""TimeReg reporting - aggregated daily hours."""
...
def generate_timereg_report(
    ...
    """Generate TimeReg report with aggregated daily hours.
    ...
    rules = get_reporting_rules(config, "timereg")
    ...
    for entry in work_entries:
        ...
        timereg_project = mapping.get("timereg")
        if not timereg_project:
            # No TimeReg mapping for this project
            continue

        project_daily_hours[timereg_project][date] += entry.duration_hours
    ...
    title = f"TimeReg Report - {week_str}"

    return format_table(headers, rows, title)


__all__ = ["generate_timereg_report"]
